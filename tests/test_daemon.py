...
class FakeBridge:
    ...
    def reply_markdown(self, message_id: str, text: str) -> str:
        # 调度器主线回复走 markdown 卡片：同样不建话题，但单独记录以便区分
        self.replies.append((message_id, text))
...
async def test_nl_dispatch_spawns_agent_via_llm():
    ...
    # 调度器回复按 markdown 卡片渲染，但仍以 om_nl 为目标、不建话题
...
async def test_nl_reply_does_not_create_thread():
    ...
    # 纯对话（无 spawn）：回复走 markdown 卡片、不建话题、不起 agent
...
async def test_scheduler_success_reply_renders_as_markdown_card():
    """调度器成功回复走 markdown 卡片（不建话题）；出错兜底仍是纯文本。"""
    daemon, bridge, created = make_daemon()
    daemon._llm = ScriptedLLM(
...
async def test_scheduler_error_fallback_stays_plain_text():
    """LLM 抛异常时走兜底纯文本回复，不套卡片（异常提示不该渲染成 markdown）。"""
    daemon, bridge, created = make_daemon()

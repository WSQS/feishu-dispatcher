...
- **Agent 控制**：ACP（JSON-RPC 2.0 over stdio），官方 `agent-client-protocol` SDK（**import 名是 `acp`**）。不要用 PTY hack。daemon 是 agent 无关的——按项目 `default_agent` 启动 `[agents]` 里配置的 argv。**Copilot（`copilot --acp`）、OpenCode（`opencode acp`）、Claude Code（`claude-agent-acp`，经 Zed 适配器 `@agentclientprotocol/claude-agent-acp`，Claude Code 无原生 ACP）均本地实测握手/流式/`load_session` 通过**；冒烟 `scripts/smoke_acp.py` / `smoke_opencode.py` / `smoke_claude.py`；Claude Code 接入详情见 `docs/claude-code-backend.md`。注意 OpenCode 与 Claude Code 支持 `session/close`（Copilot 不支持，`aclose` 里已 catch 忽略）；OpenCode 思考为逐 token 流式，`_StreamFormatter`（`acp_client.py`）只在一段连续思考的**开头**加一次 💭 合并碎片、思考转正式回复时插换行（跨 chunk 状态，见 `test_acp_extract.py`）；Claude Code 冷启动 ~15–18s（适配器+SDK 重）。
- **agent 生命周期**（review R2/R3 后的设计，改动 daemon.py 前必读）：一个 `/run` = 一个 `_AgentSession`，agent 进程与 ACP session **跨 turn 存活**（上下文在 session 里）；每 session 一个 prompt 队列 + 单消费者 worker 串行执行 turn；话题回复只入队；`/stop`（None 哨兵）、出错、**空闲超时挂起**或 daemon 退出才关闭。`AcpAgent.start()` 禁止二次调用（会抛 RuntimeError）。**per-turn 取消**：逢在途 turn（`_AgentSession.turn_in_flight`）先 `AcpAgent.cancel()` 发 ACP `session/cancel` 协作打断——在途 `prompt()` 随之带 `stop_reason="cancelled"` 返回（worker 据此安静收尾、不计完成/不通知），agent 不理会则 `prompt()` 照常跑完（优雅退化回「等整轮」）。`prompt()` 现返回 stop_reason。**两条命令共用这套机器、只差要不要终止**：`/stop` = cancel + 入队 None → 停轮**并结束** agent（标 stopped）；`/cancel` = 只 cancel、不入队 None → 停轮**保留** agent（回 idle 待命），`/cancel <新输入>` 则在 cancel 前把新输入入队（FIFO），worker 取消后即取到它作为新一轮跑（注意：是「取消 + 新一轮」，非 steering；被取消轮的进度与文件改动不回滚）。
...
- **Task status 生命周期**（worker 经 `store.update` 维护）：机械态 `starting`→`running`↔`idle`→`suspended`（自动）；**turn 中途异常 → `failed`（可恢复，非终止）**；语义终止态只剩 `done`/`stopped`（人/调度器）。`/stop` 标 `stopped`（保留历史，不删）；`suspended`/`idle`/`failed` 都可 `load_session` 惰性恢复——**`failed` = turn 异常「卡住等恢复」而非「死了」**：turn 失败时 session 已建、多半能接回，话题回复 / `send_to_task` 即尝试恢复（存 `Task.error_message` 诊断，`/task`·`/agents`·`get_task` 展示，`/agents` 单列「⚠️ 异常暂停」段，一轮成功即清空）；恢复失败（session 真过期）才停在 `failed`（startup 失败无 session，天然挡回 `/run`）。`failed` 不自动清理（同 `suspended`）。真正终止态（`done`/`stopped`）**不自动恢复**（回复提示重开）。`_shutdown` 把活跃 Task 标 `suspended` 以便重启后状态准确。终止历史留最近 `keep_terminal`（默认 50）+ `clear_terminal`。
...
- **输出转发**：`stream_mode` 二选一（config，默认 `card`）。`card`（`livecard.py`）——每回合一张 interactive 卡片,随输出 PATCH 原地更新(5 QPS/条、无编辑次数上限)，顶部状态灯(🔄/✅/❌/🛑)，body 用**卡片 JSON 2.0**(`schema:"2.0"` + `body.elements`)的 **`markdown` 组件**渲染(旧版 `lark_md`/旧版 markdown 组件只支持代码块;标题/表格/引用**只有 2.0 富文本组件**才渲染,实测确认),超 25KB 滚动到新卡片；footer 用小字号 markdown(`text_size:notation`)；`text`（`throttler.py`）——每 ~500ms 批次发一条新文本消息(兜底)。两模式经 `_AgentSession.current_channel` 间接层做到每回合独立、对 worker 透明。状态类消息(🚀/▶️/✅/❌)始终走纯文本。

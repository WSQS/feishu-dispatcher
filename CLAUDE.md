...
- 安装依赖：`uv sync`
- 测试：`uv run pytest -q`（172 个，含 daemon 生命周期 + 任务系统 + 审计/收尾回复/模型显示+切换集成测试）

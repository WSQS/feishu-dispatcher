...
- **max_agents 名额 + 空闲挂起**：`max_agents`（默认 3）限活跃 session 数。worker 的 `queue.get()` 带 `idle_timeout`（默认 1800s，`<=0` 关闭）——超时挂起：关进程腾名额，但**保留** `sessions.json` 记录（区别于 `/stop` 删除），回复即 `load_session` 恢复。上限检查与 `_launch` 登记之间**不能有 await**（否则并发 `/run` TOCTOU 突破上限）。
- **会话跨重启恢复**（`store.py`）：session 建成即把 `thread_root_id → {project, agent, session_id, cwd}` 落盘 `sessions.json`（config 同目录）；重启后话题回复到达而无活跃 session 时，用 `AcpAgent(resume_session_id=...)` 走 ACP `load_session` 惰性重连（load 期间 `_ClientImpl.set_suppress(True)` 抑制历史重放）；`/stop` 删记录。`session_id` 是 agent 专属，记录必带 agent 名。在途 turn 不恢复（只恢复会话上下文）。

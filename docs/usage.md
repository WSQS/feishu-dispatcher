...
- **等 bg 时会立刻挂起 agent**（#88）：当轮结束后若队列空且仍有在飞 bg，不等 `idle_timeout` 就腾出名额；job 完成时自动 `load_session` 唤回。`/agents` 对这类挂起任务标「等 bg」。
...
一个 agent 空闲超过 `idle_timeout`（默认 30 分钟）没新指令，daemon 会自动挂起它：关掉子进程腾出并发名额，但**会话记录保留**。你下次在话题里回复，它会自动恢复。想关掉这行为把 `idle_timeout` 设 `<=0`（等后台任务时的立刻挂起不受此项关闭，见上「后台任务」）。

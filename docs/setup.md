...
本机装好 [`uv`](https://docs.astral.sh/uv/)，以及至少一个 coding agent CLI——都经 ACP 协议控制。除标为 experimental 的 ZCode 社区 bridge 外，其余后端已本地实测握手/流式/会话恢复：
...
- **ZCode**：无原生 ACP，经非官方 [zcode-open-bridge](https://github.com/tizerluo/zcode-open-bridge) 接入；先安装并登录 ZCode，再配置 bridge。它当前标为 experimental，且默认 `mode=yolo`。完整安装、安全边界与冒烟见 [zcode-backend.md](zcode-backend.md)。

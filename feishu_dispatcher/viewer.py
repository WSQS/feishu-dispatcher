...
async def file(ctx: dict, request: dict) -> tuple[int, dict]:
    """``GET /api/projects/{name}/file?path=&rev=work``：读工作区文件内容（决策 D8）。

    返回 ``{path, rev, binary, content}``。``binary=true`` 时 ``content`` 为空串（安卓端
    提示不可预览）；文本按 UTF-8 解码，失败亦当 binary。路径过 ``resolve_under_root``（D9）。
    """
    name = request["segments"]["name"]

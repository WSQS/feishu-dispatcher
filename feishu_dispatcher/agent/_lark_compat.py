...
——事件 JSON 由 :mod:`feishu_dispatcher.channel.agent.implementation.feishu` 手写 dict 解析，
不依赖任何 lark API model。

调用约定：任何要 ``import lark_oapi`` 的模块，必须在最顶部先 ``import
feishu_dispatcher.agent._lark_compat``（顺序敏感）。重复 import 无副作用；
Feishu Channel 实现包已在其 ``__init__`` 统一安装，子模块无需各自处理。
"""

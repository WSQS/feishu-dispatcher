...
5. 用户要**新增一个项目**（给出项目名/路径/用哪个 agent）→ register_project(name,
   ...
   （种子项目删不了；只在用户明确要删时用）。
6. 用户问某项目的 issue / PR（如「有哪些 open issue」「#55 合了没、CI 过没」），或想
   了解某任务对应的工作追踪 → 用 list_forge_items（列 issue+PR，省略 project 则列所有
   项目）与 get_forge_item(project, kind, number)（看单个详情，kind 为 issue 或 pr）。
   **这两个是只读的**——你只读 forge 了解情况，绝不写 issue/PR、不改代码。
...
## 输出格式（你的回复会按 markdown 渲染到飞书，务必遵守）
- **简洁优先**：单任务操作（spawn/send/resume/mark_done）一句话带过，如
  「已启动 t22 · feishu-dispatcher · opencode」，不要附整张任务表。
- **多任务汇总用 markdown 表格**，列对齐，常用列：ID / 项目 / Agent / 状态。例：
  | ID | 项目 | Agent | 状态 |
  |----|------|-------|------|
  | t9 | SuFei | opencode | idle |
- **按状态分组**，每组一个小标题，空组省略（不要列空表）：
  **活跃任务**（running/idle/starting）→ **⚠️ 异常**（error/额度耗尽）→
  **⏸ 暂停**（suspended）→ **✅ 已终止**（done/stopped/failed）。
- **错误/命令输出用代码块**（``` 包裹）与正文区分；工具返回的冗余 JSON 只摘关键信息，
  不要原样贴回。
- 控制总长度，避免超长消息被飞书截断；列表很长时只列近况 + 给出数量汇总。"""
...
def build_scheduler_tools(
    *,
    ...
    async def _list_forge(args: dict[str, Any]) -> str:
        project = str(args.get("project", "")).strip()  # 空 = 所有已注册项目
        state = str(args.get("state", "open")).strip() or "open"
        if state not in ("open", "closed", "all"):
            return "state 必须为 open / closed / all。"
        try:
            limit = int(args.get("limit", 20))
        except (TypeError, ValueError):
            limit = 20
        limit = max(1, min(limit, 50))
        return await list_forge(project, state, limit)

    async def _get_forge(args: dict[str, Any]) -> str:
        project = str(args.get("project", "")).strip()
    ...
    return [
        ...
        ToolSpec(
            ...
            description=(
                ...
                "用户要**基于某 issue 派活**时带上 issue 编号——daemon 会自动取该 issue "
                "完整正文当 brief 交给 agent 并关联到任务，你**不必**先自己读 issue 再粘贴。"
            ),
            parameters={
                "type": "object",
                "properties": {
                    ...
                    "issue": {
                        "type": "integer",
                        "description": (
                            "可选：关联的 issue 编号；给了则用该 issue 正文当 brief 并锚到任务。"
        ...
        ToolSpec(
            name="list_forge_items",
            description=(
                "列出某项目关联仓库的 issue 和 PR（GitHub/GitLab，**只读**）。issue 与 "
                "PR 一起返回、各带 type 标记。project 省略则列出所有已注册项目的（会注明"
                "查了哪些、跳过哪些无绑定的）。用于回答「有哪些 issue/PR」等状态问题。"
            ),
            parameters={
                "type": "object",
                "properties": {
        ...
        ToolSpec(
            name="get_forge_item",
            description=(
                "查看某项目仓库里单个 issue 或 PR 的详情（标题/状态/正文/最近评论；PR "
                "另含 CI 检查、评审结论、改动量，**只读**）。需要 kind 区分 issue 还是 pr。"
            ),
            parameters={
                "type": "object",
                "properties": {

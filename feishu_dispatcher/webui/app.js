...
const storageKeys = Object.freeze({
    ...
    cursor: (conversationId) => `feishu-dispatcher.http-channel.cursor.${conversationId}`,
    started: (conversationId) => `feishu-dispatcher.http-channel.started.${conversationId}`,
});
...
function renderSelectedTask() {
    ...
    const threadId = selectedTaskId === DISPATCHER_TASK_ID
        ? null
...
function createEventArticle({ role = "assistant", label, text, detail = "", }) {
...
function appendEvent({ taskId = selectedTaskId, role = "assistant", label, text, detail = "", }) {
...
function renderEvent(event) {
    ...
    switch (event.type) {
        ...
        case "session.event": {
            ...
            if (sessionEvent.type === "agent.output.delta" ||
...
function traceRecordArticle(record, state) {
    ...
    switch (event.type) {
        ...
        case "tool.call.observed": {
            ...
            const toolDetail = typeof payload.detail === "string" && payload.detail !== title
                ? `：${payload.detail}`
                : "";
            return createEventArticle({
                role: "system",
        ...
        case "session.state.changed":
            return createEventArticle({
                role: "system",
                label: "Session",
                text: `${payload.previous_state || "unknown"} → ${payload.current_state || "unknown"}`,
                detail,
            });
...
async function loadTaskHistory(taskId, { before = null, generation = taskHistoryGeneration } = {}) {
    if (taskId === DISPATCHER_TASK_ID) {
        return;
    }
    const state = traceState(taskId);
    ...
    try {
        ...
        const payload = await apiRequest(`/api/tasks/${encodeURIComponent(taskId)}/events?${query}`);
        if (generation !== taskHistoryGeneration || selectedTaskId !== taskId) {
            return;
        }
...
async function loadChildren(dirPath) {
    ...
    const payload = await apiRequest(`/api/projects/${encodeURIComponent(selectedProject)}/tree/children?${query}`);
    treeChildrenCache.set(treeChildrenKey(dirPath), Array.isArray(payload.entries) ? payload.entries : []);
}
...
async function openFile(path) {
    ...
    try {
        ...
        const payload = await apiRequest(`/api/projects/${encodeURIComponent(selectedProject)}/file?${query}`);
...
async function connect() {
    ...
    await loadProjects();
    connected = true;
    setStatus(instanceState === "changed"
...
async function createTaskConversation(task) {
    ...
    const payload = await apiRequest(`/api/tasks/${encodeURIComponent(task.task_id)}/conversations`, {
        method: "POST",
...
async function selectTask(taskId) {
    ...
    try {
        ...
        if (historyGeneration === taskHistoryGeneration &&
            selectedTaskId === taskId) {
            throw error;
        }
        return;
    ...
    setStatus(taskIsTerminal(task)
        ? `已打开 ${taskName(task)} 的历史`
        : `已选择 ${taskName(task)}`, "ok");
}
...
function recoverPollingError(error) {
    ...
    if (!["unknown_conversation", "cursor_invalid", "cursor_expired"].includes(errorCode)) {
        return "unhandled";
    }
    ...
    const loop = async () => {
        while (generation === taskPollGeneration && connected) {
            ...
            try {
                ...
                if (generation === taskPollGeneration &&
...
function bindDivider(divider) {
    divider.addEventListener("pointerdown", (event) => {
        ...
        const onMove = (moveEvent) => {
            ...
            const nextWidth = edge === "tasks" ? startWidth + delta : startWidth - delta;
